...
def get_extension_db_path() -> str:
    ...
    if _extension_db_path is None:
        raise RuntimeError("Extension paths not initialized. Call init_extension_paths() first.")
...
class ExtensionDB:
    ...
    def create_conversation(
        ...
        try:
            ...
            """, (
                ...
                model, prompt_name, history_length, now, now
            ))
            conn.commit()
    ...
    def get_conversation(
        ...
        try:
            ...
            # Get messages
            cursor.execute("""
                SELECT message_id, role, content, page_context, created_at
    ...
    def update_conversation(
        ...
        **updates
        ...
        allowed_fields = {
            'title', 'is_temporary', 'model', 'prompt_name',
            'history_length', 'summary', 'settings_json'
        }
    ...
    def add_message(
        ...
        try:
            cursor.execute("""
                ...
                (message_id, conversation_id, role, content, page_context, created_at)
                VALUES (?, ?, ?, ?, ?, ?)
            """, (message_id, conversation_id, role, content, page_context_json, now))
            ...
            if cursor.fetchone():
                ...
                if core_updates:
                    ...
                    if 'auto_save' in core_updates:
                        core_updates['auto_save'] = 1 if core_updates['auto_save'] else 0
...
class ExtensionConversation:
    ...
    @classmethod
    def load(cls, conversation_id: str, user_email: str, db: ExtensionDB) -> Optional['ExtensionConversation']:
    ...
    @classmethod
    def create(cls, user_email: str, db: ExtensionDB, **kwargs) -> 'ExtensionConversation':
    ...
    def get_history_for_llm(self, limit: int = None) -> List[Dict]:
        ...
        recent_messages = self.messages[-n:] if n else self.messages
...
def keyParser_for_extension() -> Dict:
    ...
    if (keyStore["vllmUrl"].strip() != "" or 
        keyStore["vllmLargeModelUrl"].strip() != "" or 
        keyStore["vllmSmallModelUrl"].strip() != ""):
        try:
