...
> **Status (2026-06-09): F2 + F3 DONE; F1 (scheduling) + F4 (notifications) pending.** `utils.decay_dormant_claims(db, config, user_email=None, now=None)` flips `active`→`dormant` when `0.5 ** (age/half_life) < config.dormancy_threshold`, reading `last_reinforced_at` (fallback `updated_at`) and reusing C's `recency_half_life_days`/`half_life_by_type`. Skips pinned, `config.dormancy_exempt_types`, and non-positive half-lives. **Inert by default** (`dormancy_threshold == 0` short-circuits). Wired into `maybe_expire_claims(db, user_email, config)` (lazy, alongside hard-TTL expiry, same `EXPIRY_CHECK_INTERVAL` guard) and exposed as `StructuredAPI.run_decay_sweep()` for a future F1 job. F3: `ClaimStatus.DORMANT` already added (H); `default_search_statuses` omits it, `all_visible_statuses` now includes it (browsable/revivable). Config knob `dormancy_exempt_types` (to_dict/from_dict). Tests: `tests/test_decay.py` (9). **Remaining:** F1 (move the lazy sweep to a periodic background job) and F4 (UI notifications for newly-dormant / soon-to-expire claims).

- **F1. Scheduled sweep.** Run `expire_stale_claims()` (hard TTL) on a periodic background job (APScheduler or a server cron task) so expiry isn't traffic-dependent. *(The dormancy half is ready — `run_decay_sweep()` is the entry point such a job would call.)*
...
4. **M4 (truth depth):** D1 (supersession) + E1 (provenance). **F2 dormancy decay + F3 status DONE 2026-06-09** (the second consumer of `last_reinforced_at`).
