...
> **Status (2026-06-10): B1–B4 DONE.** New `search/ann_vector_index.py` — `VectorIndex` with two pluggable backends: **`flat`** (default; a single vectorized numpy/BLAS matmul over a normalized embedding matrix — *exact*, same ranking as the linear scan, no new dependency) and **`hnsw`** (optional faiss graph for approximate sub-linear search, auto-falling back to `flat` when faiss is missing). **B1 decision:** `sqlite-vec`/`sqlite-vss`/`hnswlib` are not installed in the env (and can't be added without network), so the index is in-process: `flat` is the safe exact default and faiss (which *is* available) backs the optional `hnsw` mode — this also sidesteps the per-user index-file lifecycle the original challenge called out. **B2:** indexes build lazily from the A1 embedding cache and are cached per-user in-process (`_INDEX_CACHE`), rebuilt when a cheap **staleness signature** (embedding count + latest `created_at`) changes — covering add/edit/delete without an explicit maintenance hook (the "rebuild if missing + checksum" mitigation). **B3:** `EmbeddingSearchStrategy._ann_search` is a fast path that over-fetches `k * ann_overfetch` from the index then re-applies the SQL filters via `_load_claims_by_ids` (status/domain/type/validity/user preserved exactly); it returns `None` to fall back to the exhaustive linear scan whenever the index is unavailable, below `ann_min_claims`, or yields no hits. It engages only above `ann_min_claims` (default 200), so small corpora — the unit tests and the 49-claim eval — stay on the exact linear path and rankings are unchanged. Config: `ann_enabled` / `ann_backend` / `ann_min_claims` / `ann_overfetch`. **B4 benchmark** (`tests/eval/benchmark_vector_index.py`, synthetic, dim 1536, k 20): flat is **~13× faster** than the Python loop at 50k vectors (82.8ms → 6.3ms); hnsw is faster still but its recall is low on *uniform random* vectors (adversarial for graph indexes — real embeddings cluster). Tests: `tests/test_vector_index.py` (11). **A3 fully unblocked:** the conflict scan can now go sub-linear via the index.

- **B1. Choose an index. ✅** see status above (in-process `flat`/`hnsw`).
- **B2. Build/maintain the index** from the A1 embedding cache; update on add/edit/delete. **✅** lazy build + staleness-signature rebuild.
- **B3. Wire `embedding_search.py`** to query the index; keep the linear path as a fallback behind a flag. **✅** `_ann_search` fast path + linear fallback, gated by `ann_enabled`/`ann_min_claims`.
- **B4. Benchmark** at 1k/10k/50k claims; record in the eval harness (G7). **✅** `benchmark_vector_index.py`.
...
- **G2-task. Batch enrichment. ✅ DONE 2026-06-10.** `add_claim`'s auto-extract path previously made ~5-6 LLM calls per claim (`extract_single`'s 4-5 field calls + a separate `generate_possible_questions` call). It now routes through a single combined `analyze_claim_statement` call (type/domain/tags/entities/possible_questions together) and reuses those questions — one call instead of six, so interactive latency *drops*. Gated by `combined_enrichment` (default True; False = legacy multi-call path). Bulk: `LLMHelpers.batch_analyze` fans the combined calls across the shared parallel executor; `add_claims_bulk` pre-computes all analyses in one parallel batch and injects each into `add_claim` via the private `_analysis` kwarg (N concurrent combined calls instead of N×6 sequential). The `auto_extract=False` path (tests + eval harness) is unchanged. Tests: `tests/test_batch_enrichment.py` (6).
- **G3-task. Export/import + audit log. ✅ DONE 2026-06-10.** Per-user JSON export/import + an append-only audit log. Schema bumped to **v10** (`audit_log` table + `idx_audit_log_user`; `_migrate_v9_to_v10`, verified on a copy of the real DB — v6→v10, 49 claims preserved, idempotent, original untouched). New `truth_management_system/portability.py`: `export_user_data` (claims/claim_links/entities/tags/contexts + join rows; embeddings excluded), `import_user_data` (`mode='merge'` `INSERT OR IGNORE`, re-owns rows to the importer, `PRAGMA defer_foreign_keys=ON` for order-independent FK resolution), and `record_audit`/`get_audit_log` (append-only). `StructuredAPI.export_data()`/`import_data(payload, mode)`/`get_audit_log(...)`; audit hooks on `add_claim`/`edit_claim`/`delete_claim`/`import_data`. REST `GET /pkb/export`, `POST /pkb/import`, `GET /pkb/audit`. Tests: `tests/test_portability.py` (9).
...
2. **M2 (scale): ✅ DONE 2026-06-10.** B1–B4 (vector index — `flat`/`hnsw` backends, per-user cache + staleness rebuild, linear fallback, benchmark) + A3 (scan cap is now `conflict_scan_limit`, sub-linear via the index). **With M2 done, every milestone (M1–M5) in this plan is complete.**
...
5. **M5 (lifecycle & polish):** **D2/D3 (consolidation + entity resolution) + F1/F4 (scheduled sweep + notifications) + G3-task (export/import + audit log) + G2-task (combined/batch enrichment) + the D1 distiller-contradiction follow-up all DONE 2026-06-10.** H4 safeguards already complete (the `reinforce_claim` contested/superseded guard returns a conflict-review failure rather than a silent boost; the C1b new-claim grace floor is in the recency re-rank). **M5 is complete.** With **Workstream B (vector index) DONE 2026-06-10**, all planned work in this document is complete.
