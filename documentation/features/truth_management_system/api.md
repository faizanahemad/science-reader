...
| **v1.0 (schema v12)** | Short-Term Cross-Conversation Memory: `pkb_short_term_memory` table; `last_accessed_at` on claims; STM CRUD (`add_short_term_memory`, `get_active_short_term_memories`, `reinforce_short_term_memory`, `promote_short_term_memory`, `delete_short_term_memory`, `expire_short_term_memories`, `touch_short_term_memories`, `touch_claims_accessed`); dual-output extraction (`_extract_short_term_memories`); auto-injection in `_get_pkb_context`; reinforcement detection + auto-promotion; compaction in `run_memory_cleanup`; REST: `GET /pkb/stm`, `POST /pkb/stm/<id>/promote`, `DELETE /pkb/stm/<id>`; UI collapsible section + compaction report. See `short_term_memory.md`. |
