...
> - Portability & audit log (schema v10): per-user JSON export/import (`portability.py`, `export_user_data`/`import_user_data`, `defer_foreign_keys` merge import, embeddings excluded), append-only `audit_log` of add/edit/delete/import (`record_audit`/`get_audit_log`), REST `GET /pkb/export`, `POST /pkb/import`, `GET /pkb/audit`
> - Combined/batch enrichment (G2): `add_claim` auto-extract uses one combined `analyze_claim_statement` call instead of ~6 (`combined_enrichment`, default on); `LLMHelpers.batch_analyze` + `add_claims_bulk` fan the calls out in parallel
> - Distiller contradiction → supersede (D1 follow-up): `llm_helpers.detect_contradiction` returns 3-way classification (`"supersedes"` / `"temporal_update"` / `"none"`). Supersedes proposes user-confirmed retire-and-replace; temporal_update adds as new claim keeping both; none preserves original relation. Used by `_detect_contradictions()` on high-similarity matches and by the rejection cache escape hatch (`distiller_detect_contradictions`, default on)
> - Rejection cache (`_filter_recently_rejected`): suppresses re-proposing claims rejected/dismissed in last 7 days. Embedding similarity > 0.92 triggers suppression, with LLM contradiction check to allow temporal updates and superseding facts through. Uses `config.embedding_model`.
> - Vector index (Workstream B): `search/ann_vector_index.py` accelerates embedding search with a `flat` (exact numpy, default) or `hnsw` (faiss, optional) backend, per-user cached with a staleness checksum; `EmbeddingSearchStrategy` over-fetches + re-applies filters above `ann_min_claims` and falls back to the exact linear scan otherwise (~13× faster at 50k vectors)
