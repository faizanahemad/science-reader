...
> - Distiller contradiction → supersede (D1 follow-up): `llm_helpers.detect_contradiction` + `ConversationDistiller` propose a user-confirmed supersede when an extracted claim replaces a conflicting existing one (`distiller_detect_contradictions`, default on)
