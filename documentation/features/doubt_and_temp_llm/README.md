...
**Document-list entry shape (dict vs tuple) — SQLite vs legacy JSON:**
- Legacy JSON-backed conversations store `message_attached_documents_list`/`uploaded_documents_list` entries as tuples `(doc_id, doc_storage, doc_source)`. SQLite-backed conversations (`conversation.db` present) return **dicts** (`{doc_id, doc_storage, doc_source, ...}`) from `ConversationStore.get_documents()` via `_get_field_sqlite`.
- `_get_attached_doc_images()`/`_get_attached_doc_texts()` previously indexed entries as tuples (`entry[0]`/`entry[1]`), which raised a `KeyError: 0` on dict entries that was swallowed by the broad `except`, so they returned `[]`. **Net effect: attached images/text docs silently never reached the doubt LLM on any SQLite-migrated (newer) conversation; older JSON conversations worked.**
- Fix: a shared `Conversation._doc_entry_fields(entry)` helper normalizes either shape to `(doc_id, doc_storage, doc_source, display_name)`. It is used everywhere these lists are read as tuples: `_get_attached_doc_images`, `_get_attached_doc_texts`, `_inject_display_attachments`, `get_message_attached_documents`, `delete_message_attached_document`, the rebuild loop in `add_message_attached_document` (where it also prevents `set_field(overwrite=True)` from dropping prior dict entries, which would otherwise wipe earlier attachments on each new attach), plus the `uploaded_documents_list` family (`get_uploaded_documents`, `add_uploaded_document`, `delete_uploaded_document`, `replace_uploaded_document`, `promote_message_attached_document`, `list_messages` doc summary), `canonical_docs.cleanup` (where a swallowed `KeyError` could have excluded live docs from the referenced set during GC), and `code_common/tools.py` (`document_lookup` / `docs_list_conversation_docs`).
- **Temp-LLM flow:** `temporary_llm_action()` inlines attached text docs via the same `_get_attached_doc_texts()` helper, so it is fixed by the same change. Temp-LLM intentionally does not send images (`images=[]`), so there was no image path to fix there.
