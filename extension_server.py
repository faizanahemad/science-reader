...
OCR_VISION_MODEL = os.getenv("EXT_OCR_MODEL", "google/gemini-2.5-flash-lite")
OCR_MAX_IMAGES = int(os.getenv("EXT_OCR_MAX_IMAGES", "30"))
OCR_MAX_WORKERS = int(os.getenv("EXT_OCR_MAX_WORKERS", "8"))
...
try:
    from extension import (
        ...
        init_extension_paths,
        keyParser_for_extension,
    )
...
def instantiate_extension_agent(
    ...
    if agent_name == "InstructionFollowingAgent":
        return InstructionFollowingAgent(
            keys, model_name=model_name, detail_level=detail_level, timeout=90
        )
    if agent_name == "JinaSearchAgent":
        return JinaSearchAgent(
            keys, model_name=model_name, detail_level=detail_level, timeout=120
        )
    ...
    if agent_name == "InterviewSimulator":
        return InterviewSimulatorAgent(
            keys,
    ...
    if agent_name == "InterviewSimulatorV2":
        return InterviewSimulatorAgentV2(
            keys,
            writer_model=model_name,
            conversation_id=conversation_id,
            detail_level=detail_level,
    ...
    if agent_name == "CodeSolveAgent":
        return CodeSolveAgent(
            keys, writer_model=model_name, n_steps=max(2, detail_level or 2)
        )
    if agent_name == "MLSystemDesignAgent":
        return MLSystemDesignAgent(
            keys, writer_model=model_name, n_steps=detail_level or 4
        )
...
def build_agent_prompt_from_history(
    history: List[Dict], page_context: Optional[Dict]
) -> str:
    """
...
def ext_get_prompt(prompt_name):
    ...
    try:
        ...
        if allowlist and prompt_name not in allowlist:
            return jsonify(
                {"error": f"Prompt '{prompt_name}' not available for extension"}
            ), 404
        ...
        try:
            ...
            return jsonify(
                {"name": prompt_name, "content": content, "raw_content": content}
            )
...
def ext_list_memories():
    ...
    try:
        ...
        result = api.claims.list(
            filters=filters if filters else None, limit=limit, offset=offset
        )
...
def ext_search_memories():
    ...
    try:
        ...
        if result.success:
            results = [
                {"claim": serialize_claim(r.claim), "score": r.score}
...
def ext_list_conversations():
    ...
    try:
        ...
        offset = request.args.get("offset", 0, type=int)
        include_temporary = (
            request.args.get("include_temporary", "true").lower() == "true"
        )

        db = get_extension_db()
        conversations = db.list_conversations(
            email, limit=limit, offset=offset, include_temporary=include_temporary
        )
...
def ext_create_conversation():
    ...
    try:
        ...
        if data.get("delete_temporary", True):  # Default to true
            ...
            if deleted_count > 0:
                logger.info(
                    f"Deleted {deleted_count} temporary conversations for {email}"
                )
        ...
        return jsonify(
            {"conversation": conv.to_dict(), "deleted_temporary": deleted_count}
        )
...
def ext_save_conversation(conversation_id):
    ...
    try:
        ...
        if success:
            conv = ExtensionConversation.load(conversation_id, email, db)
            return jsonify(
                {"conversation": conv.to_dict(), "message": "Conversation saved"}
            )
        else:
...
def ext_chat(conversation_id):
    ...
    try:
        ...
        if prompt_error:
            logger.warning(
                f"Invalid prompt '{prompt_name}', falling back to default: {prompt_error}"
            )
        ...
        logger.info(
            f"Page context received: hasContent={bool(page_context.get('content') if page_context else False)}, "
...
Please use the screenshot to answer my questions. Base your response on what you can see in the image.""",
                ...
                messages.append(
                    {
                        "role": "assistant",
                        "content": "I can see the screenshot of the page. I'll analyze the visible content to answer your questions. Note that some text may not be perfectly readable depending on image quality. What would you like to know?",
                    }
                )
                ...
                if len(page_content) > max_content_size:
                    if is_multi_tab and tab_count > 1:
                        ...
                        logger.info(
                            f"Multi-tab content truncated: {tab_count} tabs, ~{chars_per_tab} chars each"
                        )
                    else:
                        # Single page: simple truncation
                        page_content = (
                            page_content[:max_content_size]
                            + "\n\n[Content truncated...]"
                        )
...
Please use the above page content to answer my questions. Base your response on the actual content from this page."""
                ...
                logger.info(
                    f"Adding page context to messages: isMultiTab={is_multi_tab}, tabCount={tab_count}, msgLength={len(page_context_msg)}"
                )
                ...
                messages.append(
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": "Images attached for the user's latest message.",
                            }
                        ]
                        + image_parts,
                    }
        ...
        if agent_name and agent_name != "None":
            agent = instantiate_extension_agent(
                agent_name, model, keys, conversation_id, detail_level=detail_level
            )
            if not agent:
                return jsonify(
                    {"error": f"Agent '{agent_name}' could not be initialized"}
                ), 500
            ...
            if agent_name == "PromptWorkflowAgent" and workflow_steps:
                ...
                agent_prompt = build_agent_prompt_from_history(
                    conv.get_history_for_llm(), page_context
                )
            ...
            def _iter_agent_chunks(result):
                if isinstance(result, dict):
                    ...
                    yield text
                elif inspect.isgenerator(result) or isinstance(
                    result, (types.GeneratorType, collections.abc.Iterator)
                ):
                    for chunk in result:
        ...
        if stream:
            ...
            def generate():
                ...
                try:
                    for chunk in call_llm(
                        keys=keys, model_name=model, messages=messages, stream=True
                    ):
                        full_response.append(chunk)
                        yield f"data: {json.dumps({'chunk': chunk})}\n\n"
            ...
            response = call_llm(
                keys=keys, model_name=model, messages=messages, stream=False
            )
...
def _ocr_single_image(
    index: int, image_data_url: str, model: str, keys: dict
) -> Dict[str, Any]:
    """
