...
class TextIngestionDistiller:
    ...
    def __init__(
        self, api: StructuredAPI, keys: Dict[str, str], config: PKBConfig = None
    ):
        """
    ...
    def ingest_and_propose(
        ...
        if use_llm_parsing and self.keys.get("OPENROUTER_API_KEY"):
            candidates = self._parse_text_with_llm(
                text, default_claim_type, default_domain
            )
        else:
            candidates = self._parse_text_simple(
                text, default_claim_type, default_domain
            )
    ...
    def execute_plan(
        self, plan: TextIngestionPlan, approved_proposals: List[Dict]
    ...
    def _parse_text_with_llm(
        self, text: str, default_type: str, default_domain: str
...
Response:"""
        ...
        try:
            response = call_llm(
                self.keys, self.config.llm_model, prompt, temperature=0.0
            )
            ...
            try:
                ...
                if array_match:
                    ...
                    logger.warning(
                        f"No JSON array found in LLM response, falling back to simple parsing"
                    )
                    return self._parse_text_simple(text, default_type, default_domain)
    ...
    def _infer_claim_type(self, text: str, default: str) -> str:
        ...
        if any(
            ...
            kw in lower
            for kw in ["decided", "will", "going to", "plan to", "chose", "selected"]
        ):
            return "decision"
        elif any(
            kw in lower
            for kw in ["remember", "remind", "don't forget", "appointment", "deadline"]
        ):
            return "reminder"
        elif any(
            kw in lower
            ...
            kw in lower
            for kw in ["task", "todo", "need to", "should", "must", "have to"]
        ):
            return "task"
        elif any(
            kw in lower
            for kw in ["noticed", "observed", "seems", "appears", "realized"]
        ):
            return "observation"
        elif any(
            kw in lower for kw in ["remember when", "back in", "years ago", "used to"]
        ):
            return "memory"
    ...
    def _enrich_candidates(
        self, candidates: List[IngestCandidate]
    ) -> List[IngestCandidate]:
        """Enrich parsed candidates with tags, entities, and possible_questions
        using the shared analyze_claim_statement() method with an expensive model.

        Only enriches the first 20 candidates to avoid excessive API costs.
        """
        try:
            from ..llm_helpers import LLMHelpers
            from common import CHEAP_LLM
        except ImportError:
            logger.warning("Could not import models for enrichment, skipping")
            return candidates

        expensive_model = CHEAP_LLM[0] if CHEAP_LLM else self.config.llm_model
        llm = LLMHelpers(self.keys, self.config)

        max_enrich = 20
        for i, candidate in enumerate(candidates[:max_enrich]):
            try:
                analysis = llm.analyze_claim_statement(
                    candidate.statement, model=expensive_model
                )
                if analysis.claim_type and analysis.claim_type != "fact":
                    candidate.claim_type = analysis.claim_type
                if analysis.context_domain and analysis.context_domain != "personal":
                    candidate.context_domain = analysis.context_domain
                candidate.tags = analysis.tags or []
                candidate.entities = analysis.entities or []
                candidate.possible_questions = analysis.possible_questions or []
            except Exception as e:
                logger.warning(f"Enrichment failed for candidate {i}: {e}")

        return candidates

    def _find_matches_for_candidate(
    ...
    def _determine_action(
        self, candidate: IngestCandidate, matches: List[Tuple[Claim, float]]
    ) -> IngestProposal:
        """
        ...
        if not matches:
            return IngestProposal(
                action="add", candidate=candidate, reason="New fact to add"
            )
    ...
    def _generate_summary(
        self, total: int, add_count: int, edit_count: int, skip_count: int
    ) -> str:
    ...
    def _execute_proposal(
        ...
        if proposal.action == "add":
            ...
            tags = proposal.candidate.tags or []
            possible_questions = proposal.candidate.possible_questions or []
            pq_json = _json.dumps(possible_questions) if possible_questions else None
            entities = proposal.candidate.entities or []
            ...
            return ActionResult(
                ...
                action="skip",
                object_type="claim",
                object_id=proposal.existing_claim.claim_id
