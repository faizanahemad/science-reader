...
> Note: an earlier shared-connection `SQLITE_MISUSE` under bulk parallel
> embedding is now fixed (`PKBDatabase` reentrant lock), so keyed bulk runs work
> with default parallelism. If you still want fully deterministic timing you can
> construct the `EvalRunner` with `PKBConfig(max_parallel_embedding_calls=1,
