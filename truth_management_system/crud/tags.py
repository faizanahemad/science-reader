...
from ..models import Tag, Claim, TAG_COLUMNS
...
class TagCRUD(BaseCRUD[Tag]):
    ...
    def add(self, tag: Tag) -> Tag:
        ...
        to the tag if not already set.  If the tag has no friendly_id,
        one is auto-generated with the _tag suffix (v0.7).
        ...
        if not tag.friendly_id:
            from ..utils import generate_tag_friendly_id

            tag.friendly_id = generate_tag_friendly_id(tag.name)
        ...
        with self.db.transaction() as conn:
            ...
            actual_cols = [c for c in TAG_COLUMNS if c in db_columns]

            columns = ", ".join(actual_cols)
            placeholders = ", ".join(["?" for _ in actual_cols])
            values = tuple(getattr(tag, k) for k in actual_cols)

            conn.execute(
                f"INSERT INTO tags ({columns}) VALUES ({placeholders})", values
            )

            logger.debug(f"Added tag: {tag.tag_id} ({tag.name}, fid={tag.friendly_id})")

        return tag

    def edit(self, tag_id: str, patch: Dict[str, Any]) -> Optional[Tag]:
        """
    ...
    def get_or_create(
        ...
        if existing:
            # Backfill friendly_id if missing (pre-v7 tag)
            if not existing.friendly_id:
                from ..utils import generate_tag_friendly_id

                fid = generate_tag_friendly_id(name)
                self.edit(existing.tag_id, {"friendly_id": fid})
    ...
    def find_by_name_and_parent(
        self, name: str, parent_tag_id: Optional[str] = None
    ) -> Optional[Tag]:
        """
    ...
    def find_by_name(self, name_pattern: str, limit: int = 100) -> List[Tag]:
        """
    ...
    def get_by_friendly_id(self, friendly_id: str) -> Optional[Tag]:
        """
        Get a tag by its user-facing friendly_id.

        If user_email is set on this CRUD instance, the lookup is scoped
        to that user's tags only.

        Args:
            friendly_id: The user-facing alphanumeric identifier (ends with _tag).
        ...
        if self.user_email:
            row = self.db.fetchone(
                "SELECT * FROM tags WHERE friendly_id = ? AND user_email = ?",
                (friendly_id, self.user_email),
            )
        else:
            row = self.db.fetchone(
                "SELECT * FROM tags WHERE friendly_id = ?", (friendly_id,)
            )
        return Tag.from_row(row) if row else None
    ...
    def search_friendly_ids(self, prefix: str, limit: int = 10) -> List[Tag]:
        ...
        Returns:
            List of tags matching the prefix.
        """
        if not prefix:
            return []
    ...
    def resolve_claims(
        ...
        """
        Recursively get all claims with this tag and all descendant tags.

        This is used when a user references @tag_friendly_id in chat.
        It traverses the tag hierarchy (parent → children → grandchildren)
        and collects all claims linked to any tag in the tree.

        If user_email is set, only returns claims owned by that user.

        Args:
            tag_id: ID of the root tag to resolve.
        ...
        # Use recursive CTE to get all tag IDs in hierarchy
        sql = f"""
            WITH RECURSIVE tag_tree AS (
                SELECT tag_id, 0 as depth FROM tags WHERE tag_id = ?
                UNION ALL
        ...
        params = [tag_id, max_depth] + statuses + user_params + [limit]
        rows = self.db.fetchall(sql, tuple(params))
        return [Claim.from_row(row) for row in rows]

    def move(self, tag_id: str, new_parent_id: Optional[str]) -> Optional[Tag]:
        """
